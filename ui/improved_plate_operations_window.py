...
from core.polygon_operations import (
    join_plate_features_by_intersect,
    join_plate_features_by_union,
    split_plate_features_by_difference,
)
...
class PlateSplittingTabWidget(ProcessTabWidget):
    ...
    def setup_ui(self):
        ...
        self.splitter_selection.currentIndexChanged.connect(
    ...
    def validate_and_enable_process(self):
        ...
        self.process_button.setEnabled(
            has_splitter and has_time and has_features and has_output
        )
    ...
    def process_splitting(self):
        ...
        try:
            ...
            splitter_model_index = self.splitter_model.index(
                splitter_index, FeatureDataColumn.feature_id
            )
            splitter_feature_id = self.splitter_model.data(splitter_model_index)
            splitter_fc_name = self.splitter_model.data(
                self.splitter_model.index(
                    splitter_index, FeatureDataColumn.feature_collection
                )
            )
            ...
            splitter_fc = next(
                filter(
                    lambda x: x.shortname == splitter_fc_name,
                    self.session.loaded_feature_collections,
                )
            ).feature_collection
            splitter_feature = splitter_fc.get(
                lambda f: f.get_feature_id().get_string() == splitter_feature_id
            )

            if not splitter_feature:
                QMessageBox.critical(
                    self, "Error", "Could not find selected splitting feature"
                )
                return
            ...
            if len(result_fc) == 0:
                QMessageBox.information(
                    self, "No Results", "Plate splitting returned no features"
                )
                return
...
class LineSplittingTabWidget(ProcessTabWidget):
    ...
    def setup_ui(self):
        ...
        self.steps = [
            ProcessStepWidget(
                1, "Select First Line", "Choose the first line feature to split"
            ),
            ProcessStepWidget(
        ...
        self.line1_selection.currentIndexChanged.connect(
            lambda: self.update_step_status(0, True)
        )
        ...
        self.line2_selection.currentIndexChanged.connect(
            lambda: self.update_step_status(1, True)
        )
        ...
        self.split_time.setValidator(QDoubleValidator())
        self.split_time.setPlaceholderText("e.g., 10.0")
        self.split_time.textChanged.connect(
    ...
    def validate_and_enable_process(self):
        ...
        self.process_button.setEnabled(
            has_line1 and has_line2 and has_time and has_output
        )
    ...
    def process_line_splitting(self):
        ...
        try:
            ...
            line1_model_index = self.line1_model.index(
                line1_index, FeatureDataColumn.feature_id
            )
            line1_feature_id = self.line1_model.data(line1_model_index)
            line1_fc_name = self.line1_model.data(
                self.line1_model.index(
                    line1_index, FeatureDataColumn.feature_collection
                )
            )

            line1_fc = next(
                filter(
                    lambda x: x.shortname == line1_fc_name,
                    self.session.loaded_feature_collections,
                )
            ).feature_collection
            line1_feature = line1_fc.get(
                lambda f: f.get_feature_id().get_string() == line1_feature_id
            )

            line2_model_index = self.line2_model.index(
                line2_index, FeatureDataColumn.feature_id
            )
            line2_feature_id = self.line2_model.data(line2_model_index)
            line2_fc_name = self.line2_model.data(
                self.line2_model.index(
                    line2_index, FeatureDataColumn.feature_collection
                )
            )

            line2_fc = next(
                filter(
                    lambda x: x.shortname == line2_fc_name,
                    self.session.loaded_feature_collections,
                )
            ).feature_collection
            line2_feature = line2_fc.get(
                lambda f: f.get_feature_id().get_string() == line2_feature_id
            )

            if not line1_feature:
                QMessageBox.critical(
                    self, "Error", "Could not find selected first line"
                )
                return
            if not line2_feature:
                QMessageBox.critical(
                    self, "Error", "Could not find selected second line"
                )
                return
            ...
            if len(result_fc) == 0:
                QMessageBox.information(
                    self, "No Results", "No line intersections found"
                )
                return
...
class PolygonOperationsTabWidget(ProcessTabWidget):
    ...
    def setup_ui(self):
        ...
        self.steps = [
            ProcessStepWidget(
                1,
                "Select Polygon Features",
                "Choose polygon features for geometric operation",
            ),
            ProcessStepWidget(
                2, "Set Split Time", "Define the split time for operation"
            ),
            ProcessStepWidget(
                3,
            ...
            ProcessStepWidget(
                4, "Configure Parameters", "Set operation parameters and output options"
            ),
            ProcessStepWidget(
                5, "Process & Save", "Execute operation and save results"
            ),
        ]
        ...
        self.intersection_button.clicked.connect(
            lambda: self.select_operation("intersection")
        )
        ...
        self.difference_button.clicked.connect(
            lambda: self.select_operation("difference")
        )
        ...
        self.plate_id_widget = PlateIdWidget(
            self.session, "Plate ID", enable_uniqueness_validation=False
        )
        ...
        content_layout.addWidget(self.polygon_group)
        content_layout.addWidget(self.time_group)
        content_layout.addWidget(self.operation_group)
    ...
    def on_time_changed(self):
        """Handle time range changes."""
        self.polygon_model.setStartTimeFilter(
            float(self.split_time.text()) if bool(self.split_time.text()) else None
        )
        self.update_step_status(1, bool(self.split_time.text()))
        self.validate_and_enable_process()
    ...
    def validate_and_enable_process(self):
        ...
        self.process_button.setEnabled(
            has_polygons and has_time and has_operation and has_output
        )
    ...
    def process_operation(self):
        ...
        try:
            ...
            if len(selected_features) < 2:
                QMessageBox.warning(
                    self, "Error", "Please select at least 2 polygon features"
                )
                return
            ...
            if len(result_fc) == 0:
                QMessageBox.information(
                    self,
                    "No Results",
                    f"{self.selected_operation.title()} operation returned no features",
                )
                return
...
class ImprovedPlateOperationsWindow(QWidget):
    ...
    def setup_ui(self):
        ...
        self.tab_widget.setTabToolTip(
            1, "Find and create features at line intersections"
        )
        self.tab_widget.setTabToolTip(
            2, "Perform geometric operations on polygon features"
        )
        ...
        self.tab_widget.setMinimumSize(
            self.tab_widget.currentWidget().layout().minimumSize()
        )
