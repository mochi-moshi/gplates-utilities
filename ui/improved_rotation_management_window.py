...
from core.rotations import (
    ...
    create_new_rotation_plate,
    replace_final_rotation,
)
...
class RotationInitializationWidget(ProcessTabWidget):
    ...
    def setup_ui(self):
        ...
        self.steps = [
            ProcessStepWidget(
                1,
                "Select Features for Rotation Model",
                "Choose features that will be used to create rotation model.\nSelect none to generate for all plates.",
            ),
            ProcessStepWidget(
                2, "Set Time Parameters", "Define the time period for rotation model"
            ),
            ProcessStepWidget(
                3, "Initialize & Save", "Create rotation model and save results"
            ),
        ]
    ...
    def validate_and_enable_process(self):
        ...
        self.process_button.setEnabled(
            self.time_widget._validate_times() and has_output
        )
        ...
        try:
            ...
            if not selected_features:
                ...
                selected_features = [
                    ...
                    for f in lfc.feature_collection
                ]

            if not selected_features:
                QMessageBox.warning(
                    self, "Error", "No features available for rotation model creation"
                )
                return

            # Create rotation model
            result_fc = create_initial_rotations(
                selected_features, start_time, end_time
            )
            ...
            QMessageBox.critical(
                self, "Error", f"Rotation model initialization failed:\n{str(e)}"
            )
...
class PlateCreationWidget(ProcessTabWidget):
    ...
    def setup_ui(self):
        ...
        self.steps = [
            ProcessStepWidget(
                1,
                "Select Features for New Plate",
                "Choose features that will become the new plate",
            ),
            ProcessStepWidget(
                2, "Set Plate Parameters", "Define new plate ID and time parameters"
            ),
            ProcessStepWidget(
                3,
            ...
            ProcessStepWidget(
                4, "Create Plate & Rotations", "Generate new plate and rotation entries"
            ),
        ]
        ...
        self.split_time.textChanged.connect(
            lambda: self.update_step_status(1, bool(self.split_time.text()))
        )
    ...
    def validate_and_enable_process(self):
        ...
        self.process_button.setEnabled(
            has_features and has_time and has_feature_output and has_rotation_output
        )
    ...
    def create_plate(self):
        ...
        try:
            ...
            if len(feature_fc) == 0:
                QMessageBox.warning(
                    self, "No Results", "No features created for new plate"
                )
                return
...
class RotationMaintenanceWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        self.rotation_output_path.setPlaceholderText(
            "Select rotation file output location..."
        )
    ...
    def fix_final_rotation(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Error", f"Failed to fix final rotation:\n{str(e)}"
            )
        ...
        try:
            ...
            if not rotation_features:
                self.validation_results.setText(
                    "❌ No rotation features found in the model"
                )
            ...
            for feature in rotation_features:
                ...
                if plate_id not in time_ranges:
                    ...
                    time_ranges[plate_id]["start"] = max(
                        time_ranges[plate_id]["start"], begin_time
                    )
                    time_ranges[plate_id]["end"] = min(
                        time_ranges[plate_id]["end"], end_time
                    )
            ...
            for plate_id in sorted(plate_ids):
                ...
                if plate_id in time_ranges:
                    ...
                    time_info += (
                        f"(Time range: {tr['start']:.2f} - {tr['end']:.2f} Mya)"
                    )
                else:
                    time_info += "MISSING"
            ...
            if len(missing_rotations) > 0:
                warnings.append(
                    f"⚠️  {len(missing_rotations)} plates missing rotations ({', '.join(missing_rotations)})"
                )
