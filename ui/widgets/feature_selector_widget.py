...
class FeatureSelectorWidget(QWidget):
    ...
    def setup_ui(self, single_selection: bool):
        ...
        self.plate_filter.setValidator(
            QRegularExpressionValidator(QRegularExpression("\\d+(,\\s*\\d+)*"))
        )
        ...
        self.feature_view.setItemDelegateForColumn(
            FeatureDataColumn.start_time, TimeDecoratorDelegate(self.feature_view)
        )
        self.feature_view.setItemDelegateForColumn(
            FeatureDataColumn.end_time, TimeDecoratorDelegate(self.feature_view)
        )
        self.feature_view.setSelectionMode(
    ...
    def connect_signals(self):
        ...
        self.feature_view.selectionModel().selectionChanged.connect(
    ...
    def update_name_filter(self):
        ...
        try:
            if not self.name_filter.text():
                ...
                names = [self.name_filter.text().strip()] + [
                    name.strip() for name in self.name_filter.text().split(",")
                ]
    ...
    def update_plate_filter(self):
        ...
        try:
            if self._single_plate_id:
                selected_rows = set(
                    [
                        self.feature_model.mapToSource(i).row()
                        for i in self.feature_view.selectedIndexes()
                    ]
                )
                selected_feature_data = [
                    extractFeatureDataFromRow(self.feature_model.sourceModel(), row_num)
            ...
            if filter_text:
                plate_ids = selected_ids + [
                    id.strip() for id in filter_text.split(",") if id.strip()
                ]
    ...
    def update_collection_filter(self):
        ...
        try:
            if not self.collection_filter.text():
                ...
                collections = [
                    collection.strip()
                    for collection in self.collection_filter.text().split(",")
                ]
    ...
    def get_selected_features(self):
        ...
        selected_feature_ids = [
            ...
            if i.column() == FeatureDataColumn.feature_id
        ]
        selected_feature_collections = [
            i.data()
            for i in selected_indexes
            if i.column() == FeatureDataColumn.feature_collection
        ]
        ...
        for i in range(len(selected_feature_ids)):
            fc = next(
                filter(
                    lambda x: x.shortname == selected_feature_collections[i],
                    self.session.loaded_feature_collections,
                )
            ).feature_collection
            feature = fc.get(
                lambda f: f.get_feature_id().get_string() == selected_feature_ids[i]
            )
            if feature:
