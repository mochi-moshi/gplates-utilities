...
class PlateIdWidget(QWidget):
    ...
    def __init__(
        ...
        session: Session,
        label: str = "Plate ID",
        enable_uniqueness_validation: bool = True,
        parent=None,
    ):
    ...
    def setup_ui(self):
        ...
        self.plate_id_input.valueChanged.connect(
            lambda: self.validation_timer.start(300)
        )
    ...
    def _validate_plate_id(self, plate_id: int) -> bool:
        ...
        try:
            ...
            if (
                self.enable_uniqueness_validation
                and self.session._rotationFeatureCollection is not None
            ):
                    ...
                    try:
                        ...
                        if moving_id == plate_id:
                            self.status_label.setText(
                                f"⚠️ Plate ID {plate_id} already exists in rotation model"
                            )
...
class DualPlateIdWidget(QWidget):
    ...
    def __init__(
        self, session: Session, enable_uniqueness_validation: bool = True, parent=None
    ):
        super().__init__(parent)
        self.enable_uniqueness_validation = enable_uniqueness_validation
    ...
    def setup_ui(self, session: Session):
        ...
        description_label = QLabel(
            "Features split by the rift will be assigned to left or right plates based on their position relative to the rift line:"
        )
        ...
        self.left_plate_widget = PlateIdWidget(
            session, "Left Plate ID", self.enable_uniqueness_validation
        )
        ...
        self.right_plate_widget = PlateIdWidget(
            session, "Right Plate ID", self.enable_uniqueness_validation
        )
    ...
    def get_plate_ids(self) -> tuple[int, int]:
        ...
        return (
            self.left_plate_widget.get_plate_id(),
            self.right_plate_widget.get_plate_id(),
        )
