...
class SubductionTabWidget(ProcessTabWidget):
    ...
    def setup_ui(self):
        ...
        self.steps = [
            ProcessStepWidget(
                1,
                "Select Subduction Zone",
                "Choose the subduction zone that will consume features",
            ),
            ProcessStepWidget(2, "Set Time Range", "Define when subduction occurs"),
            ProcessStepWidget(
                3,
                "Select Features to Subduct",
                "Choose features that will be processed by subduction",
            ),
            ProcessStepWidget(
                4, "Process & Save", "Execute subduction and save results"
            ),
        ]
        ...
        self.sz_selection.currentIndexChanged.connect(
            lambda: self.update_step_status(0, True)
        )
        ...
        self.output_widget.pathChange.connect(
            lambda: self.validate_and_enable_process()
        )
    ...
    def validate_and_enable_process(self):
        ...
        self.process_button.setEnabled(
            has_sz and has_time and has_features and has_output
        )
    ...
    def process_subduction(self):
        ...
        try:
            ...
            sz_fc_name = self.sz_model.data(
                self.sz_model.index(sz_index, FeatureDataColumn.feature_collection)
            )

            # Find the actual feature
            sz_fc = next(
                filter(
                    lambda x: x.shortname == sz_fc_name,
                    self.session.loaded_feature_collections,
                )
            ).feature_collection
            sz_feature = sz_fc.get(
                lambda f: f.get_feature_id().get_string() == sz_feature_id
            )

            if not sz_feature:
                QMessageBox.critical(
                    self, "Error", "Could not find selected subduction zone"
                )
                return
            ...
            if len(result_fc) == 0:
                QMessageBox.information(
                    self, "No Results", "Subduction processing returned no features"
                )
                return
            ...
            QMessageBox.critical(
                self, "Error", f"Subduction processing failed:\n{str(e)}"
            )
...
class RiftingTabWidget(ProcessTabWidget):
    ...
    def setup_ui(self):
        ...
        # Workflow steps
        self.steps = [
            ProcessStepWidget(1, "Select Rift", "Choose the continental rift feature"),
            ProcessStepWidget(2, "Set Split Time", "Define when rifting occurs"),
            ProcessStepWidget(
                3,
                "Select Features to Rift",
                "Choose features that will be split by rifting",
            ),
            ProcessStepWidget(4, "Configure Rifting", "Set rifting parameters"),
            ProcessStepWidget(5, "Process & Save", "Execute rifting and save results"),
        ]
        ...
        self.plate_ids_widget = DualPlateIdWidget(
            self.session, enable_uniqueness_validation=True
        )
        self.plate_ids_widget.plateIdsChanged.connect(
            lambda left, right: self.update_step_status(3, True)
        )
        ...
        self.feature_output_widget = OutputWidget(
            self.session,
            "GPlates Markup Language (*.gpml)",
            enable_topology_generation=True,
        )
        self.feature_output_widget.pathChange.connect(
            lambda: self.validate_and_enable_process()
        )
        ...
        self.rotation_output_widget = OutputWidget(
            self.session,
            "PLATES4 Rotation File (*.rot)",
            enable_topology_generation=False,
        )
        self.rotation_output_widget.pathChange.connect(
            lambda: self.validate_and_enable_process()
        )
    ...
    def on_rift_selected(self):
        ...
        if has_rift:
            rift_model_index = self.rift_model.index(
                self.rift_selection.currentIndex(), FeatureDataColumn.feature_id
            )
    ...
    def on_split_time_set(self):
        ...
        self.rift_model.setStartTimeFilter(
            split_time if self.split_time.text() else None
        )
        self.feature_selector.set_time_filter(
            None, split_time if self.split_time.text() else None
        )
    ...
    def validate_and_enable_process(self):
        ...
        self.process_button.setEnabled(
            ...
            and has_time
            and has_features
            and has_feature_output
            and has_rotation_output
        )
    ...
    def process_rifting(self):
        ...
        try:
            ...
            rift_model_index = self.rift_model.index(
                rift_index, FeatureDataColumn.feature_id
            )
            rift_feature_id = self.rift_model.data(rift_model_index)
            rift_fc_name = self.rift_model.data(
                self.rift_model.index(rift_index, FeatureDataColumn.feature_collection)
            )

            # Find the actual feature
            rift_fc = next(
                filter(
                    lambda x: x.shortname == rift_fc_name,
                    self.session.loaded_feature_collections,
                )
            ).feature_collection
            rift_feature = rift_fc.get(
                lambda f: f.get_feature_id().get_string() == rift_feature_id
            )
            ...
            if len(result_fc) == 0:
                QMessageBox.information(
                    self, "No Results", "Rifting processing returned no features"
                )
                return
            ...
            if self.feature_output_widget.should_append() and path.exists(
                feature_output_path
            ):
            ...
            if self.rotation_output_widget.should_append() and path.exists(
                rotation_output_path
            ):
...
class SimpleDivergenceTabWidget(ProcessTabWidget):
    ...
    def setup_ui(self):
        ...
        self.steps = [
            ProcessStepWidget(
                1, "Select Ridge", "Choose the mid-ocean ridge for spreading"
            ),
            ProcessStepWidget(
        ...
        self.ridge_selection.currentIndexChanged.connect(
            lambda: self.update_step_status(0, True)
        )
    ...
    def process_divergence(self):
        ...
        try:
            ...
            ridge_model_index = self.ridge_model.index(
                ridge_index, FeatureDataColumn.feature_id
            )
            ridge_feature_id = self.ridge_model.data(ridge_model_index)
            ridge_fc_name = self.ridge_model.data(
                self.ridge_model.index(
                    ridge_index, FeatureDataColumn.feature_collection
                )
            )
            ...
            ridge_fc = next(
                filter(
                    lambda x: x.shortname == ridge_fc_name,
                    self.session.loaded_feature_collections,
                )
            ).feature_collection
            ridge_feature = ridge_fc.get(
                lambda f: f.get_feature_id().get_string() == ridge_feature_id
            )
            ...
            if len(result_fc) == 0:
                QMessageBox.information(
                    self,
                    "No Results",
                    "Ocean spreading processing returned no features",
                )
                return
            ...
            QMessageBox.critical(
                self, "Error", f"Ocean spreading processing failed:\n{str(e)}"
            )
            traceback.print_exception(e)
...
class TripleDivergenceTabWidget(ProcessTabWidget):
    ...
    def setup_ui(self):
        ...
        self.ridgeA_selection.currentIndexChanged.connect(
            lambda: self.on_ridge_select()
        )
        ...
        self.ridgeB_selection.currentIndexChanged.connect(
            lambda: self.on_ridge_select()
        )
        ...
        self.ridgeC_selection.currentIndexChanged.connect(
            lambda: self.on_ridge_select()
        )
    ...
    def on_ridge_select(self):
        ...
        if not ridgeA_index < 0:
            feature_id = self.ridgeA_model.data(
                self.ridgeA_model.index(ridgeA_index, FeatureDataColumn.feature_id)
            )
            ab.append(feature_id)
            ac.append(feature_id)
        ridgeB_index = self.ridgeB_selection.currentIndex()
        if not ridgeB_index < 0:
            feature_id = self.ridgeB_model.data(
                self.ridgeB_model.index(ridgeB_index, FeatureDataColumn.feature_id)
            )
            ab.append(feature_id)
            bc.append(feature_id)
        ridgeC_index = self.ridgeC_selection.currentIndex()
        if not ridgeC_index < 0:
            feature_id = self.ridgeC_model.data(
                self.ridgeC_model.index(ridgeC_index, FeatureDataColumn.feature_id)
            )
            ac.append(feature_id)
            bc.append(feature_id)
    ...
    def validate_and_enable_process(self):
        """Validate inputs and enable process button if ready."""
        has_ridge = (
            self.ridgeA_selection.currentIndex() > -1
            and self.ridgeB_selection.currentIndex() > -1
            and self.ridgeC_selection.currentIndex() > -1
        )
        has_time = self.time_widget._validate_times()
        has_output, _ = self.output_widget.is_valid()
    ...
    def process_divergence(self):
        ...
        try:
            ...
            if ridgeA_index < 0:
                QMessageBox.warning(
                    self, "Error", "Please select a first mid-ocean ridge"
                )
                return

            ridgeA_model_index = self.ridgeA_model.index(
                ridgeA_index, FeatureDataColumn.feature_id
            )
            ridgeA_feature_id = self.ridgeA_model.data(ridgeA_model_index)
            ridgeA_fc_name = self.ridgeA_model.data(
                self.ridgeA_model.index(
                    ridgeA_index, FeatureDataColumn.feature_collection
                )
            )
            ...
            ridgeA_fc = next(
                filter(
                    lambda x: x.shortname == ridgeA_fc_name,
                    self.session.loaded_feature_collections,
                )
            ).feature_collection
            ridgeA_feature = ridgeA_fc.get(
                lambda f: f.get_feature_id().get_string() == ridgeA_feature_id
            )

            if not ridgeA_feature:
                QMessageBox.critical(
                    self, "Error", "Could not find first selected ridge"
                )
                return
            # Get selected ridge
            ridgeB_index = self.ridgeB_selection.currentIndex()
            ...
            ridgeB_model_index = self.ridgeB_model.index(
                ridgeB_index, FeatureDataColumn.feature_id
            )
            ridgeB_feature_id = self.ridgeB_model.data(ridgeB_model_index)
            ridgeB_fc_name = self.ridgeB_model.data(
                self.ridgeB_model.index(
                    ridgeB_index, FeatureDataColumn.feature_collection
                )
            )
            ...
            ridgeB_fc = next(
                filter(
                    lambda x: x.shortname == ridgeB_fc_name,
                    self.session.loaded_feature_collections,
                )
            ).feature_collection
            ridgeB_feature = ridgeB_fc.get(
                lambda f: f.get_feature_id().get_string() == ridgeB_feature_id
            )

            if not ridgeB_feature:
                QMessageBox.critical(
                    self, "Error", "Could not find second selected ridge"
                )
                return
            # Get selected ridge
            ridgeC_index = self.ridgeC_selection.currentIndex()
            if ridgeC_index < 0:
                QMessageBox.warning(
                    self, "Error", "Please select a third mid-ocean ridge"
                )
                return

            ridgeC_model_index = self.ridgeC_model.index(
                ridgeC_index, FeatureDataColumn.feature_id
            )
            ridgeC_feature_id = self.ridgeC_model.data(ridgeC_model_index)
            ridgeC_fc_name = self.ridgeC_model.data(
                self.ridgeC_model.index(
                    ridgeC_index, FeatureDataColumn.feature_collection
                )
            )
            ...
            ridgeC_fc = next(
                filter(
                    lambda x: x.shortname == ridgeC_fc_name,
                    self.session.loaded_feature_collections,
                )
            ).feature_collection
            ridgeC_feature = ridgeC_fc.get(
                lambda f: f.get_feature_id().get_string() == ridgeC_feature_id
            )

            if not ridgeC_feature:
                QMessageBox.critical(
                    self, "Error", "Could not find t hird selected ridge"
                )
                return
...
class ImprovedSubductionRiftHelperWindow(QWidget):
    ...
    def setup_ui(self):
        ...
        self.tab_widget.setTabToolTip(
            0, "Process feature consumption through subduction zones"
        )
        self.tab_widget.setTabToolTip(
            1, "Model continental rifting and plate splitting"
        )
        self.tab_widget.setTabToolTip(2, "Generate ocean crust from mid-ocean ridges")
        self.tab_widget.setTabToolTip(
            3, "Generate ocean crust at a triple juciton of mid-ocean ridges"
        )
