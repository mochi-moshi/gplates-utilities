...
class FeatureFilterModel(QSortFilterProxyModel):
    ...
    def filterAcceptsRow(self, row_num: int, _) -> bool:
        ...
        valid_time = (self._start_time_filter is None or data.end_time <= self._start_time_filter <= data.start_time) and (self._end_time_filter is None or self._end_time_filter <= data.start_time)
        valid_id = len(self._id_filter) == 0 or (data.reconstruction_method == 'ByPlateId' and data.plate_id in self._id_filter or data.reconstruction_method in ['HalfStageRotation', 'HalfStageRotationVersion2', 'HalfStageRotationVersion3'] and (data.left_plate in self._id_filter or data.right_plate in self._id_filter))
        valid_name = len(self._name_filter) == 0 or any(name in data.feature_name_and_id for name in self._name_filter)
        valid_geometry = len(self._geometry_filter) == 0 or any(geom == data.geometry_type for geom in self._geometry_filter)
        valid_reconstruction = len(self._reconstruction_filter) == 0 or any(recon == data.reconstruction_method for recon in self._reconstruction_filter)
        valid_collection = len(self._collection_filter) == 0 or any(collection == data.feature_collection for collection in self._collection_filter)
        valid_type = len(self._type_filter) == 0 or any(typ == data.feature_type for typ in self._type_filter)
        excluded = data.feature_id in self._excluded_features
        
        return not excluded and (valid_time and valid_id and valid_name and valid_geometry and valid_reconstruction and valid_collection and valid_type)
