...
class FeatureCollectionLoader(QWidget):
    def __init__(self, session: Session):
        ...
        self.remove_button.setEnabled(
            False
        )  # Button is disabled until a list item is selected
        self.remove_button.clicked.connect(self.on_remove)

        self.fc_list = QListView()
        self.fc_list.setModel(session.get_feature_collection_model())
        self.fc_list.setEditTriggers(
            QAbstractItemView.EditTrigger.NoEditTriggers
        )  # Do not allow editing the names
        self.fc_list.setSelectionMode(
            QAbstractItemView.SelectionMode.SingleSelection
        )  # Only one name may be selected at a time
    ...
    def on_remove(self):
        ...
        ret = QMessageBox.warning(
            ...
            f"Are you sure you want to remove '{current_item_name}' from this project?",
