...
def create_initial_rotations(
    features: list[Feature], start_time: float, end_time: float
):
...
def replace_final_rotation(rotations: FeatureCollection, end_time: float = 0):
    plate_ids = set(
        [
            f.get(PropertyName.gpml_moving_reference_frame).get_value().get_plate_id()
            for f in rotations
        ]
    )
    ...
    for plate_id in plate_ids:
        features = rotations.get(
            lambda f: f.get(PropertyName.gpml_moving_reference_frame)
            .get_value()
            .get_plate_id()
            == plate_id,
            FeatureReturn.all,
        ...
        for feature in features[1:]:
            ...
            if (
                samples[-1].get_time()
                < min_plate_time[plate_id]["samples"][-1].get_time()
            ):
                min_plate_time[plate_id] = {
                    "feature": feature,
                    "ref_id": ref_id,
                ...
                samples[-1].get_time()
                < min_plate_time[plate_id]["prev_samples"][-1].get_time()
            ):
                min_plate_time[plate_id] = {
    ...
    for plate_id, plate_data in min_plate_time.items():
        if plate_data["samples"][-1].get_time() != end_time:
            if len(plate_data["samples"]) > 1:
                plate_data["samples"].append(
                    GpmlTimeSample(plate_data["samples"][-2].get_value(), end_time)
                )
                plate_data["samples"].sort(lambda ts: ts.get_time())
                feature = fc.get(
                    lambda f: f.get_feature_id()
                    == plate_data["feature"].get_feature_id()
                )
                feature.set_total_reconstruction_pole(
                    plate_data["ref_id"], plate_id, plate_data["samples"]
                )
            else:
                plate_data["prev_samples"].append(
                    GpmlTimeSample(plate_data["prev_samples"][-1].get_value(), end_time)
                )
                plate_data["prev_samples"].sort(lambda ts: ts.get_time())
                feature = fc.get(
                    lambda f: f.get_feature_id()
                    == plate_data["prev_feature"].get_feature_id()
                )
                feature.set_total_reconstruction_pole(
                    plate_data["prev_ref_id"], plate_id, plate_data["prev_samples"]
                )
        else:
            if len(plate_data["samples"]) > 1:
                plate_data["samples"][-1] = GpmlTimeSample(
                    plate_data["samples"][-2].get_value(), end_time
                )
                plate_data["samples"].sort(lambda ts: ts.get_time())
                feature = fc.get(
                    lambda f: f.get_feature_id()
                ...
                plate_data["prev_samples"][-1] = GpmlTimeSample(
                    plate_data["prev_samples"][-1].get_value(), end_time
                )
                plate_data["prev_samples"].sort(lambda ts: ts.get_time())
                feature = fc.get(
                    lambda f: f.get_feature_id()
...
def split_rotation_features_by_plate_id(
    rotations: FeatureCollection,
    old_plate_id: int,
    new_plate_id: int,
    split_time: float,
) -> list[Feature]:
    oldest_time = split_time
    youngest_time = split_time
    for rotation in rotations:
        ...
        if rplate_id == old_plate_id:
            oldest_time = max(sequence[-1].get_time(), oldest_time)
            youngest_time = min(sequence[0].get_time(), youngest_time)

    rotationsModel = RotationModel(rotations)

    # Already relative to default anchor_plate (0)
    split_rotation = rotationsModel.get_rotation(split_time, old_plate_id)
    ...
    rc = [
        Feature.create_total_reconstruction_sequence(
            ...
            GpmlIrregularSampling(
                [
                    GpmlTimeSample(GpmlFiniteRotation(split_rotation), youngest_time),
                    GpmlTimeSample(GpmlFiniteRotation(split_rotation), split_time),
                ]
            ),
        ),
        Feature.create_total_reconstruction_sequence(
            ...
            GpmlIrregularSampling(
                [
                    GpmlTimeSample(
                        GpmlFiniteRotation(FiniteRotation.create_identity_rotation()),
                        split_time,
                    )
                ]
            ),
...
def create_new_rotation_plate(
    ...
    new_rc = split_rotation_features_by_plate_id(
        rotations, plateId, new_plate_id, split_time
    )
