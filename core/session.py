...
def create_feature_item(lfc: LoadedFeatureCollection, feature: pygplates.Feature):
    ...
    feature_item.append(
        QStandardItem(f"{feature.get_name()} ({feature.get_feature_id().get_string()})")
    )
    feature_item.append(
        QStandardItem(feature.get_feature_type().get_name())
    )  # Feature Type
    feature_item.append(
        QStandardItem(type(feature.get_geometry()).__name__)
    )  # Geometry Type
    feature_item.append(
        QStandardItem(str(feature.get_reconstruction_plate_id()))
    )  # Plate ID
    ...
    feature_item.append(
        QStandardItem(feature.get_feature_id().get_string())
    )  # Feature ID
    feature_item.append(QStandardItem(lfc.shortname))  # Feature Collection (shortname)
    feature_item.append(
        QStandardItem(feature.get_reconstruction_method())
    )  # Feature Method
...
class Session:
    def __init__(self) -> None:
        ...
        self._feature_model.setHorizontalHeaderLabels(
            [
                "Feature Name",
                "Feature Name (ID)",
                "Feature Type",
    ...
    def reload_rotation_model(self):
        ...
        self._rotationFeatureCollection = pygplates.FeatureCollection(
            self._rotationModel_path
        )
    ...
    def load_feature_collections(self, paths: list[str]) -> None:
        new_paths = [
            p
            for p in paths
            if len([lfc for lfc in self.loaded_feature_collections if lfc.path == p])
            == 0
        ]
        ...
        self._feature_collection_names.setStringList(
            [x.shortname for x in self.loaded_feature_collections]
        )

    def unload_feature_collection(self, shortname):
        lfc = next(
            filter(lambda x: x.shortname == shortname, self.loaded_feature_collections),
            None,
        )
        ...
        items = self._feature_model.findItems(
            shortname,
            Qt.MatchFlag.MatchFixedString,
            FeatureDataColumn.feature_collection,
        )
        for i in items:
