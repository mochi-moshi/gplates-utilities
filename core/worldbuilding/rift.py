...
def rift(features: list[Feature], rift: Feature, rotation_model: RotationModel, split_time: float, left_plate_id: int, right_plate_id: int, max_transition_size = 10, *, use_topologies: bool = False):
  ...
  plate_id = features[0].get_reconstruction_plate_id()
  ...
  for feature, geometry in zip(features, feature_geometries):
    ...
    if isinstance(geometry, (PointOnSphere, MultiPointOnSphere)):
      left_features, right_features = _split_point_features(
        ...
        split_time, left_plate_id, right_plate_id
      )
    ...
    # Reverse reconstruct split features to present day
...
def _determine_side_of_rift(point: PointOnSphere, rift_geometry: PolylineOnSphere) -> str:
  ...
  for i in range(len(rift_geometry) - 1):
    ...
    distance = segment_line.distance(point)
...
def _split_point_features(feature: Feature, geometry, rift_geometry: PolylineOnSphere, 
                         feature_name: str, feature_end: float, split_time: float,
                         left_plate_id: int, right_plate_id: int) -> tuple[list[Feature], list[Feature]]:
  ...
  if isinstance(geometry, PointOnSphere):
    ...
    if side == 'left':
      left_feature = Feature.create_reconstructable_feature(
        feature.get_feature_type(), geometry, f'{feature_name} Left',
        valid_time=(split_time, feature_end), reconstruction_plate_id=left_plate_id
      )
      left_features.append(left_feature)
    elif side == 'right':
      right_feature = Feature.create_reconstructable_feature(
        feature.get_feature_type(), geometry, f'{feature_name} Right',
        valid_time=(split_time, feature_end), reconstruction_plate_id=right_plate_id
      )
    ...
    if left_points:
      ...
      left_feature = Feature.create_reconstructable_feature(
        feature.get_feature_type(), left_geometry, f'{feature_name} Left',
        valid_time=(split_time, feature_end), reconstruction_plate_id=left_plate_id
      )
      left_features.append(left_feature)
    ...
    if right_points:
      ...
      right_feature = Feature.create_reconstructable_feature(
        feature.get_feature_type(), right_geometry, f'{feature_name} Right',
        valid_time=(split_time, feature_end), reconstruction_plate_id=right_plate_id
      )
      right_features.append(right_feature)
...
def _split_polyline_features(feature: Feature, geometry: PolylineOnSphere, rift_geometry: PolylineOnSphere,
  ...
  for split_geometry in split_geometries:
    ...
    if side == 'left':
      left_feature = Feature.create_reconstructable_feature(
        feature.get_feature_type(), split_geometry, f'{feature_name} Left',
        valid_time=(split_time, feature_end), reconstruction_plate_id=left_plate_id
      )
      left_features.append(left_feature)
    elif side == 'right':
      right_feature = Feature.create_reconstructable_feature(
        feature.get_feature_type(), split_geometry, f'{feature_name} Right',
        valid_time=(split_time, feature_end), reconstruction_plate_id=right_plate_id
      )
      right_features.append(right_feature)
...
def _split_polygon_features(feature: Feature, geometry: PolygonOnSphere, rift_geometry: PolylineOnSphere,
                          feature_name: str, feature_start: float, split_time: float,
                          left_plate_id: int, right_plate_id: int) -> tuple[list[Feature], list[Feature]]:
  ...
  try:
    ...
    for split_polygon in split_polygons:
      try:
        ...
        if side == 'left':
          left_feature = Feature.create_reconstructable_feature(
            feature.get_feature_type(), split_polygon, f'{feature_name} Left',
            valid_time=(split_time, feature_start), reconstruction_plate_id=left_plate_id
          )
          left_features.append(left_feature)
        elif side == 'right':
          right_feature = Feature.create_reconstructable_feature(
            feature.get_feature_type(), split_polygon, f'{feature_name} Right',
            valid_time=(split_time, feature_start), reconstruction_plate_id=right_plate_id
          )
          right_features.append(right_feature)
