...
def rift(
    ...
    left_plate_id: int,
    right_plate_id: int,
    max_transition_size=10,
    *,
    generate_mor: bool = False,
    ...
    if not all(
        f.get_reconstruction_plate_id() == plate_id
        for f in features
        if f.get_reconstruction_method() == "ByPlateId"
    ) or not all(
        f.get_left_plate() == plate_id or f.get_right_plate() == plate_id
        for f in features
        ...
        raise ValueError(
            "All features must have the same reconstruction plate ID, or Half-Stage must have one Id be the split plate"
        )
    ...
    if left_plate_id != plate_id:
        to_add.extend(
            split_rotation_features_by_plate_id(
                rotation_features, plate_id, left_plate_id, split_time
            )
        )
    if right_plate_id != plate_id:
        to_add.extend(
            split_rotation_features_by_plate_id(
                rotation_features, plate_id, right_plate_id, split_time
            )
        )
    ...
    rift_geometry = rift_snapshot.get_reconstructed_geometries()[
        0
    ...
    feature_geometries = [
        rg.get_reconstructed_geometry()
        for rg in features_snapshot.get_reconstructed_geometries(
            ...
            Feature.create_reconstructable_feature(
                ...
                other_properties=[
                    (
                        ...
                        Enumeration(
                            EnumerationType.create_gpml(
                                "ReconstructionMethodEnumeration"
                            ),
                            "HalfStageRotationVersion3",
                        ),
                    ),
                    (PropertyName.gpml_left_plate, GpmlPlateId(left_plate_id)),
                    (PropertyName.gpml_right_plate, GpmlPlateId(right_plate_id)),
                ],
...
def _determine_side_of_rift(
    point: PointOnSphere, rift_geometry: PolylineOnSphere
) -> str:
    """
...
def recreate_reconstructable_feature_with_method(
    ...
    if method == "ByPlateId":
        feature = Feature.create_reconstructable_feature(
            ...
            reconstruction_plate_id=int(new_plate_id),
            reverse_reconstruct=(rotation_model, GeoTimeInstant(split_time)),
        )
        return feature
    if method in [
        ...
        left_plate = (
            int(new_plate_id)
            if old_feature.get_left_plate() == int(old_plate_id)
            else old_feature.get_left_plate()
        )
        right_plate = (
            int(new_plate_id)
            if old_feature.get_right_plate() == int(old_plate_id)
            else old_feature.get_right_plate()
        )
        feature = Feature.create_reconstructable_feature(
            old_feature.get_feature_type(),
            new_geometry,
...
def _split_point_features(
    feature: Feature,
    geometry,
    rift_geometry: PolylineOnSphere,
    feature_name: str,
    feature_end: float,
    split_time: float,
    old_plate_id: int,
    ...
    if isinstance(geometry, PointOnSphere):
        ...
        if side == "left":
            left_feature = recreate_reconstructable_feature_with_method(
                ...
                left_plate_id,
                old_plate_id,
                valid_time=(split_time, feature_end),
        ...
        if left_points:
            left_geometry = (
                MultiPointOnSphere(left_points)
                if len(left_points) > 1
                else PointOnSphere(left_points[0])
            )
            left_feature = recreate_reconstructable_feature_with_method(
                feature,
                left_geometry,
        ...
        if right_points:
            right_geometry = (
                MultiPointOnSphere(right_points)
                if len(right_points) > 1
                else PointOnSphere(right_points[0])
            )
            right_feature = recreate_reconstructable_feature_with_method(
                feature,
                right_geometry,
...
def _split_polyline_features(
    feature: Feature,
    geometry: PolylineOnSphere,
    rift_geometry: PolylineOnSphere,
    feature_name: str,
    feature_end: float,
    split_time: float,
...
def _split_polygon_features(
    feature: Feature,
    geometry: PolygonOnSphere,
    rift_geometry: PolylineOnSphere,
    feature_name: str,
    feature_start: float,
    split_time: float,
            ...
            try:
                ...
                if side == "left":
                    left_feature = recreate_reconstructable_feature_with_method(
                        ...
                        left_plate_id,
                        old_plate_id,
                        valid_time=(split_time, feature_start),
                        rotation_model=rotation_model,
                        split_time=split_time,
                    )
                    left_features.append(left_feature)
