...
from ..utils import (
    ...
    lines_equal,
    is_valid,
    get_all_reconstruction_plate_ids,
)
...
def subduct(
    ...
    *,
    ...
    valid_features = [
        f for f in features if is_valid(start_time, end_time, f.get_valid_time())
    ]
    ...
    for older_time, younger_time in times:
        ...
        snapshots_older = ReconstructSnapshot(
            valid_features, rotation_model, older_time
        ).get_reconstructed_geometries(same_order_as_reconstructable_features=True)
        snapshots_younger = ReconstructSnapshot(
            valid_features, rotation_model, younger_time
        ).get_reconstructed_geometries(same_order_as_reconstructable_features=True)
        ...
        for feature, snap_older, snap_younger in zip(
            valid_features,
            [s.get_reconstructed_geometry() for s in snapshots_older],
            [s2.get_reconstructed_geometry() for s2 in snapshots_younger],
        ):
            ...
            zone_frames, ref_zone_frames = create_zone_frames(
                subduction_zone, rotation_model, older_time, younger_time, feature
            )
...
def create_zone_area(
    subduction_zone: Feature,
    rotation_model: RotationModel,
    older_time: float,
    younger_time: float,
    reference_feature: Feature = None,
) -> tuple[PolygonOnSphere, PolygonOnSphere]:
    """Create zone area polygon from subduction zone at two time points."""
    sz1 = (
        ReconstructSnapshot([subduction_zone], rotation_model, older_time)
        .get_reconstructed_geometries()[0]
        .get_reconstructed_geometry()
    )
    sz2 = (
        ReconstructSnapshot([subduction_zone], rotation_model, younger_time)
        .get_reconstructed_geometries()[0]
        .get_reconstructed_geometry()
    )
    ...
    if reference_feature is not None:
        feature_by_ref = recreate_reconstructable_feature_with_method(
            reference_feature, sz1, ""
        )
        reverse_reconstruct([feature_by_ref], rotation_model, older_time)
        sz3 = (
            ReconstructSnapshot([feature_by_ref], rotation_model, younger_time)
            .get_reconstructed_geometries()[0]
            .get_reconstructed_geometry()
        )
...
def create_zone_frames(
    subduction_zone: Feature,
    rotation_model: RotationModel,
    older_time: float,
    younger_time: float,
    reference_feature: Feature = None,
) -> tuple[list[PolygonOnSphere], list[PolygonOnSphere]]:
    """Create zone area polygon from subduction zone at two time points."""
    sz1 = (
        ReconstructSnapshot([subduction_zone], rotation_model, older_time)
        .get_reconstructed_geometries()[0]
...
def process_point_geometry(
    feature: Feature,
    snap_older,
    snap_younger,
    zone_frames: list[PolygonOnSphere],
    ref_zone_frames: list[PolygonOnSphere],
    feature_name: str,
    feature_start: float,
    feature_end: float,
    time2: float,
    ...
    paths = (
        [PolylineOnSphere([snap_older, snap_younger])]
        if geo_type == "PointOnSphere"
        else [
            PolylineOnSphere([p1, p2]) if p1 != p2 else p2
            for p1, p2 in zip(snap_older, snap_younger)
        ]
    )
    ...
    for path, point in zip(paths, points):
        alive = not any(
            zone.partition(path)
            in [PartitionResult.intersecting, PartitionResult.inside]
            for zone in zone_frames
        )
        alive = alive and not any(
            zone.is_point_in_polygon(point) for zone in ref_zone_frames
        )
        alive.append(alive)
        ...
        if alive[0]:
            to_reconstruct_and_process.append(
                recreate_reconstructable_feature_with_method(
                    feature,
                    PointOnSphere(snap_younger),
                    f"{feature_name} Alive",
...
def process_polyline_segments_with_zone(
    snap_older, snap_younger, zone_frames: list[PolygonOnSphere]
) -> tuple[list, list]:
    """Process polyline segments for subduction, returning valid and invalid segment paths."""
    segment_paths = [
        (
            s2,
            PolylineOnSphere([s2.get_start_point(), s2.get_end_point()]),
            PolygonOnSphere(
                [
                    s1.get_start_point(),
                    s1.get_end_point(),
        ...
        for s1, s2 in zip(snap_older.get_segments(), snap_younger.get_segments())
    ]
    ...
    for seg, seg_line, path in segment_paths:
        start_path = next(
            (path for p, path in point_paths if p == seg.get_start_point())
        )
        ...
        for zone in zone_frames:
            start_subducted = zone.partition(start_path) in [
                PartitionResult.intersecting,
                PartitionResult.inside,
            ]
            end_subducted = zone.partition(end_path) in [
                PartitionResult.intersecting,
                PartitionResult.inside,
            ]
            subducted = zone.partition(path) in [
                PartitionResult.intersecting,
                PartitionResult.inside,
            ]
...
def process_polylines_with_ref_zone(
...
def calculate_subzone_path(
    snap_younger: PolygonOnSphere,
    zone_frames: list[PolygonOnSphere],
    ref_zone_frames: list[PolygonOnSphere],
):
    """Calculate and add subzone path based on closest distance to valid segment."""

    zone_outline = PolylineOnSphere.join(
        [PolylineOnSphere(zone_frames[0][:3][::-1])]
        + [PolylineOnSphere(zone[:2][::-1]) for zone in zone_frames[1:-1]]
        + [
            PolylineOnSphere(zone_frames[-1][:2][::-1]),
            PolylineOnSphere([zone_frames[-1][0], zone_frames[-1][-1]]),
        ]
    )
    ref_zone_outline = PolylineOnSphere.join(
        [PolylineOnSphere(ref_zone_frames[0][:3][::-1])]
        + [PolylineOnSphere(zone[:2][::-1]) for zone in ref_zone_frames[1:-1]]
        + [
            PolylineOnSphere(ref_zone_frames[-1][:2][::-1]),
            PolylineOnSphere([ref_zone_frames[-1][0], ref_zone_frames[-1][-1]]),
        ]
    )
    ...
    for i in range(len(polygon_lines)):
        for subzone_line in subzone_lines:
            if polygon_lines[i][0] == subzone_line[0]:
                polygon_lines[i] = PolylineOnSphere(
                    subzone_line[::-1][:-1] + polygon_lines[i][:]
                )
            elif polygon_lines[i][0] == subzone_line[-1]:
                polygon_lines[i] = PolylineOnSphere(
                    subzone_line[:][:-1] + polygon_lines[i][:]
                )
            elif polygon_lines[i][-1] == subzone_line[0]:
                polygon_lines[i] = PolylineOnSphere(
                    polygon_lines[i][:] + subzone_line[:][1:]
                )
            elif polygon_lines[i][-1] == subzone_line[-1]:
                polygon_lines[i] = PolylineOnSphere(
                    polygon_lines[i][:] + subzone_line[::-1][1:]
                )
...
def process_polygon_geometry(
    ...
    pre_valid_segment_paths, invalid_segment_paths = (
        process_polyline_segments_with_zone(snap_older, snap_younger, zone_frames)
    )
    valid_segment_paths, extra_invalid_segment_paths = process_polylines_with_ref_zone(
        pre_valid_segment_paths, ref_zone_frames
    )
    ...
    invalid_polygon_lines = PolylineOnSphere.join(
        invalid_segment_paths + extra_invalid_segment_paths
    )
...
def process_polyline_geometry(
    ...
    pre_valid_segment_paths, pre_invalid_segment_paths = (
        process_polyline_segments_with_zone(snap_older, snap_younger, zone_frames)
    )

    valid_segment_paths, invalid_segment_paths = process_polylines_with_ref_zone(
        pre_valid_segment_paths, ref_zone_frames
    )
    ...
    invalid_lines = PolylineOnSphere.join(
        invalid_segment_paths + pre_invalid_segment_paths
    )
    ...
    for line in invalid_lines:
        to_reconstruct_and_add.append(
            recreate_reconstructable_feature_with_method(
                feature, line, f"{feature_name} Dead", valid_time=(feature_start, time2)
            )
        )
