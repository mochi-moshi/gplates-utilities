...
def generate_time_steps(rotation_features: FeatureCollection, plate_ids: set[int] | list[int], start_time: float, end_time: float, fixed_too: bool = True) -> list[tuple[float, float]]:
  ...
  if fixed_too:
      new_ids = set([fixed for fixed, moving, _ in (f.get_total_reconstruction_pole() for f in rotation_features) if moving in plate_ids and fixed not in plate_ids])
      while new_ids:
          plate_ids = set(plate_ids).union(new_ids)
          new_ids = set([fixed for fixed, moving, _ in (f.get_total_reconstruction_pole() for f in rotation_features) if moving in new_ids and fixed not in new_ids])
